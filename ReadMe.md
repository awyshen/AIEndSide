...
|---------|------|
| 测试数据集 | 50个意图识别测试用例 |
| 硬件平台 | GPU: NVIDIA RTX 3090 / CPU: Intel i7-12700K |
| 量化工具 | GGUF (Q4_K_M, Q8_0) |
| 评估指标 | 准确率、推理速度 |

### 7.2 模型测试结果

| 模型名称 | 是否量化 | 测试通过率(50) | 推理速度 | 运行环境 |
|---------|---------|---------------|---------|---------|
| Qwen3.5-0.8B | 否 | 49/50 (98%) | - | GPU |
| Qwen3-0.6B | 否 | 27/50 (54%) | - | GPU |
| Qwen3.5-2B | 否 | 47+2/50 (94%+2) | - | GPU |
| Qwen3.5-4B | 否 | **50/50 (100%)** | - | GPU |
| MiniCPM5-1B | 否 | 37/50 (74%) | - | GPU |
| Qwen3-1.7B-GGUF:Q8_0 | 是 | 39/50 (78%) | 20+ t/s | CPU |
| Qwen3.5-2B-Q4_K_M.gguf | 是 | 41+3/50 (82%+6%) | 20+ t/s | CPU |
| Qwen3.5-4B-Q4_K_M.gguf | 是 | 45+1/50 (90%+2%) | 20+ t/s | CPU |
| MiniCPM5-1B-Q4_K_M.gguf | 是 | 30/50 (60%) | 30+ t/s | CPU |

### 7.3 模型选型建议

| 场景 | 推荐模型 | 理由 |
|-----|---------|------|
| **端侧部署** | Qwen3.5-4B-Q4_K_M.gguf | 90%+准确率，20+ t/s速度，CPU运行 |
| **高性能要求** | Qwen3.5-4B | 100%准确率，GPU加速 |
| **极致轻量化** | Qwen3.5-0.8B | 98%准确率，最小模型体积 |
| **CPU优先** | Qwen3.5-2B-Q4_K_M.gguf | 88%准确率，20+ t/s速度 |

### 7.4 测试结论

1. **Qwen3.5系列表现最优**: 在相同参数量级下，Qwen3.5模型明显优于Qwen3和MiniCPM5
2. **量化影响**: 4-bit量化后准确率下降约5-10%，但获得20+ t/s的CPU推理速度
3. **端侧推荐**: Qwen3.5-4B-Q4_K_M.gguf是端侧部署的最佳选择，兼顾准确率和速度
4. **当前选型**: 本项目采用Qwen3.5-0.8B作为默认模型，平衡性能与资源占用
